...
function StartaProgres(text, duration, linear)
    if type(text) ~= "string" then
        PrintError("text inconnue")
        return
    end

    if tonumber(duration) == nil then
        PrintError("durée inconnue")
        return
    end
...
function aMiniGameCustom(options, static)
    -- ERROR HANDLING --
    ...
    if options.Async == false then
        while Run do
            ...
            if IsControlJustPressed(0, cancelKey) and options.canCancel then
                OnComplete(true)
                print("Gagner")
                TriggerEvent("aProgressBar:stop")
            end
...
function Linear(text, duration)
    StartaProgres(text, duration, true)
end
...
function Static(config)
    local options = aMiniGameCustom(config, true)
...
function aAttenteMiniGame(options)
    if Run then
        return
    end
...
RegisterNetEvent("aProgressBar:start")
RegisterNetEvent("aProgressBar:stop")
RegisterNetEvent("aProgressBar:custom")
RegisterNetEvent("aProgressBar:linear")
RegisterNetEvent("aProgressBar:minigame")

AddEventHandler("aProgressBar:start", Start)
AddEventHandler("aProgressBar:stop", Stop)
AddEventHandler("aProgressBar:custom", function(options)
    options.Async = false
    options.onStart = nil
    options.onComplete = nil
...
AddEventHandler("aProgressBar:linear", Linear)
AddEventHandler("aProgressBar:minigame", MiniGame)
